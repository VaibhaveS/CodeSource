...
async function parseTree(repoName, sha_url, userName, accessToken) {
  const response = JSON.parse(
    await httpGet(
      "/repos/" + userName + "/" + repoName + "/git/trees/" + sha_url,
      accessToken
    )
  );
  const files = {};
  for (let i = 0; i < response.tree.length; i++) {
    ...
    if (item.type === "blob") {
      ...
      const subFiles = await parseTree(
        repoName,
        item.sha,
        userName,
        accessToken
...
router.post("/directoryTree", async function (req, res) {
  ...
  return res.status(200).send({
    ...
    "src/main/resources/application.properties":
      "05da2efdc7abb784f491beeb0c3863bda3375b75",
    ...
    "src/test/resources/features/todoBatch.feature":
      "2d896393dfbd92d3202f92a513967492802c228c",
  });
  const accessToken = await getAccessToken(req.user.userId);
  const body = JSON.parse(
    await httpGet(
      "/repos/" + req.user.details.username + "/" + req.body.repoLink,
      accessToken
    )
  );
